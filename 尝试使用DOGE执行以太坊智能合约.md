...
ecdsa 签名的 recoverid 允许 0,1,2,3 四个值，大概率会是0，1 这两个值是符合 智能合约中的 ecrecover 函数的，小概率可能会出现 3，4 的情况（我并没有遇到这种情况），如果你想避免出现3，4 以至于会令用户重新操作的情况，可以采用预言机的办法在智能合约之外部署签名查询验证器。如果你使用预言机部署签名验证器，事实上避免了需要中间验证地址的情况。
